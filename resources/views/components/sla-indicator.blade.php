@props(['ticket' => null])

@php
    if (!$ticket || !$ticket->sla_due_at) {
        $status = 'no_sla';
        $colorClass = 'bg-gray-100 text-gray-800 border-gray-200';
        $icon = 'question-mark-circle';
        $text = 'No SLA';
    } else {
        $slaService = app(\App\Services\SlaCalculatorService::class);
        $slaStatus = $slaService->getSlaStatus($ticket);

        switch ($slaStatus['status']) {
            case 'breached':
                $colorClass = 'bg-red-100 text-red-800 border-red-200';
                $icon = 'exclamation-circle';
                $text = 'SLA Breached';
                break;
            case 'critical':
                $colorClass = 'bg-red-50 text-red-700 border-red-300';
                $icon = 'exclamation-triangle';
                $text = 'Critical (' . round($slaStatus['remaining_percentage']) . '% left)';
                break;
            case 'warning':
                $colorClass = 'bg-yellow-100 text-yellow-800 border-yellow-200';
                $icon = 'clock';
                $text = 'Warning (' . round($slaStatus['remaining_percentage']) . '% left)';
                break;
            case 'on_track':
                $colorClass = 'bg-green-100 text-green-800 border-green-200';
                $icon = 'check-circle';
                $text = 'On Track (' . round($slaStatus['remaining_percentage']) . '% left)';
                break;
            case 'on_time':
                $colorClass = 'bg-blue-100 text-blue-800 border-blue-200';
                $icon = 'check-circle';
                $text = 'On Time';
                break;
            default:
                $colorClass = 'bg-gray-100 text-gray-800 border-gray-200';
                $icon = 'question-mark-circle';
                $text = 'Unknown';
        }
    }
@endphp

@if($ticket && $ticket->sla_due_at)
    <div class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border {{ $colorClass }}">
        @if($icon === 'exclamation-circle')
            <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"></path>
            </svg>
        @elseif($icon === 'exclamation-triangle')
            <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path>
            </svg>
        @elseif($icon === 'clock')
            <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
            </svg>
        @elseif($icon === 'check-circle')
            <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path>
            </svg>
        @else
            <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path>
            </svg>
        @endif
        <span>{{ $text }}</span>
        @if($ticket->sla_due_at)
            <span class="ml-1 text-xs opacity-75">
                ({{ $ticket->sla_due_at->format('M j, H:i') }})
            </span>
        @endif
    </div>
@else
    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border bg-gray-100 text-gray-800 border-gray-200">
        <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
            <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path>
        </svg>
        {{ $text }}
    </span>
@endif
